...
How **jtf-meridian** fits the wider Cognis suite — a composable set of defensive, analytical
tools that all run on your own hardware and speak plain **JSON** (or an OpenAI-compatible
**`/v1`**). `jtf-meridian` lives in the **Private-AI backbone** cluster.
...
| from | relation | to |
|---|---|---|
| `jtf-meridian` | JSON findings compose with | [`edgemesh`](https://github.com/cognis-digital/edgemesh), [`modelroute`](https://github.com/cognis-digital/modelroute), [`uncensored-fleet`](https://github.com/cognis-digital/uncensored-fleet), [`cognis-code`](https://github.com/cognis-digital/cognis-code) |
| `jtf-meridian` | AI add-ins are served `/v1` by | [`edgemesh`](https://github.com/cognis-digital/edgemesh) (your fleet) |
| `jtf-meridian` | findings can be narrated / reasoned over by | [`humind`](https://github.com/cognis-digital/humind) -> [`agentlex`](https://github.com/cognis-digital/agentlex) |
| `jtf-meridian` | export to intel formats via | [`stixgen`](https://github.com/cognis-digital/stixgen) (STIX) / [`attackmap`](https://github.com/cognis-digital/attackmap) (ATT&CK) |

## Composition patterns

Everything reads/writes JSON, so tools chain with ordinary pipes; nothing leaves the box.

**1 — chain within the private-ai backbone cluster.** `jtf-meridian` output feeds the next tool:
```bash
jtf-meridian ... --format json > out.json          # this tool's findings (see `jtf-meridian --help`)
...
jtf-meridian ...                                         # vision / reasoning add-ins light up
```

**3 — export findings to your SOC's formats.**
```bash
jtf-meridian ... --format json | stixgen from-json > bundle.stix.json   # STIX 2.1
jtf-meridian ... --format json | attackmap map > attack.json            # ATT&CK techniques
```

**4 — narrate through cognition + agents.** `humind` extracts salience from `jtf-meridian`'s
output; `agentlex` holds it as KB facts and fires Horn rules to escalate. `jtf-meridian` slots
